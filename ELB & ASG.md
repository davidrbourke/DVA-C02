...
- Network load balancer: TCP, TLS, UDP (OSI Layer 4)
...
3. To prevent direct access to the ECS instances, so they can only be access via the ALB, update the EC2 Instances shared Security Group to remove the public access on Port 80.
...
- EC2 Instances
- IP Address (must be private - if running the virtual applicances on your own network/data centre)
...
- In the Load Balancer Listener:
  - You must specifiy a default certificate
...
- Any custom metrics that can be pushed to Cloud Watch.
